...
| ----- | --------------------------------------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| #55   | M5-01 Integrate sql.js runtime and DB open service              | ✅ Fully implemented | WASM loading configured via [sqlJsLoader.ts](file:///c:/Users/Jonas/Repositories/Conspectus-Mobile/src/db/sqlJsLoader.ts) with Vite `?url` asset resolution. Lifecycle (open/close/exec/export) implemented in [browserDbRuntime.ts](file:///c:/Users/Jonas/Repositories/Conspectus-Mobile/src/db/browserDbRuntime.ts) with foreign key pragma enforcement, SQLite header validation, deterministic error normalization via [dbRuntimeErrors.ts](file:///c:/Users/Jonas/Repositories/Conspectus-Mobile/src/db/dbRuntimeErrors.ts), and final pre-assignment supersession protection fixed in `review-fixing`.            |
...
| M2        | ✅ No regressions | Dual deployment channels and build verifier coverage remain intact. Production smoke security-header verification remains explicitly deferred to M8-01 and is now documented in `docs/CI-CD-Pipelines.md`; see M-05.                                                                         |
...
| M4        | ✅ No regressions | Dexie-backed cache store, startup freshness decisions, progress indicators, and sync state transitions are preserved. Cache/runtime handoff now validates downloaded snapshots before cache promotion and protects final runtime assignment in `review-fixing`; see M-06 and M-07.           |
...
- **Status:** ✅ Fixed in `review-fixing` by documented deferral to M8-01
- **Severity:** Medium
...
- **Resolution:** The deferral is now documented in `docs/CI-CD-Pipelines.md`, and the workflow variable is explicitly tied to `M8-01`, which owns adding the required website-hosted response headers and flipping the smoke check back to strict mode.
...
- **Supersession Guard:** The `canApply` pattern in `BrowserDbRuntimeOpenOptions` is the right design for overlapping async sync operations, and `review-fixing` adds the final pre-assignment guard so superseded replacement opens cannot overwrite the accepted runtime; see M-06.
...
- **Sync & Cache Architecture:** The Dexie-backed cache, in-memory SQLite runtime, and file download/validate pipeline provide the right foundation. The `browserDbRuntime.exportBytes()` method is tested and ready for upload, and the runtime/cache hardening in M-06 and M-07 is fixed in `review-fixing`.
...
- **Runtime/Cache Hardening:** Fixed in `review-fixing`; startup/cache invariants now keep the previous accepted runtime/cache snapshot intact when a replacement is superseded or fails full SQLite/Conspectus validation.
...
| Medium    | 7     | 0        | 0    | 0      | 0   | 1           | 6      |
| Large     | 0     | 0        | 0    | 0      | 0   | 0           | 0      |
| **Total** | 18    | 0        | 0    | 0      | 0   | 1           | 17     |
