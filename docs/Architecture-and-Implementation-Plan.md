...
- The current free production host ignores the optional `.htaccess` response headers. On 2026-07-22, the application owner accepted the resulting MVP limitation: production retains HSTS and the document-level CSP/referrer policy, but does not receive runtime `Content-Security-Policy`, `X-Content-Type-Options`, or `Referrer-Policy` headers. This is explicit residual-risk acceptance rather than equivalent technical enforcement and must be reconsidered when hosting capabilities change.
